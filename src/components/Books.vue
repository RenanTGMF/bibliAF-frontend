<template>
  <div class="p-6 bg-gradient-to-b from-blue-50 to-blue-200 min-h-screen">
    ...
    <confirm-modal
      :isOpen="isOverdueLoan"
      :title="overdueLoans === 1 ? `Você tem ${overdueLoans} empréstimo vencido`:` Você tem ${overdueLoans} empréstimos vencidos`"
      :message="'Para renovar o empréstimo, entre em contato com o administrador.'"
      @close="closeModal"
    />
    ...
    <div class="mt-6 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      ...
      >
        <img 
          :src="book.image" 
          alt="Capa do Livro" 
          class="w-full h-full object-cover rounded-t-md md:rounded-l-md md:rounded-tr-none md:w-48 mx-auto"
        />
        <div class="p-4 flex-1 flex flex-col justify-between">
          <div>
            ...
            <p class="text-gray-600 font-medium mt-2">Disponíveis: {{ book.copiesAvailable }}/{{ book.copies }}</p>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      userReservations: [], 
      loans: [],
      overdueChecked: localStorage.getItem('overdueChecked') || false,
    };
  },
  created() {
    this.fetchBooks();
    this.fetchUserReservations();
    if(!this.overdueChecked){
  ...
  methods: {
    ...
    async checkOverdueLoans() {
      localStorage.setItem('overdueChecked', true);
      ...
      if (overdueLoans.length > 0) {
        this.isOverdueLoan = true;
        this.overdueLoans = overdueLoans.length;
      }
    },
    closeModal() {
      this.isOverdueLoan = false;
