      ...
      <table class="min-w-full bg-white border border-gray-300 rounded-lg shadow-lg">
        <thead class="bg-gray-200 text-center">
          <tr>
            ...
            <th @click="sort('reservationDate')" class="cursor-pointer p-4">Data de Reserva <span v-if="sortKey === 'reservationDate'">{{ sortOrder === 'asc' ? '↑' : '↓' }}</span></th>
        ...
        <tbody>
          <tr v-for="reservation in sortedReservations" :key="reservation.id" class="hover:bg-gray-100">
            ...
            <td class="p-4 border-b text-center">
              <button @click.stop="confirmCancelReservation(reservation.id)" class="bg-primary text-white px-4 py-2 rounded-md hover:bg-opacity-80 ml-2">Cancelar</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-if="reservations.length !== 0" class="lg:hidden mt-6 grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      ...
      >
        ...
        <div class="text-white text-center cursor-pointer flex flex-row justify-around">
          <button class="flex-1" @click.stop="confirmCancelReservation(reservation.id)">
            <div class="p-4 bg-primary text-white text-center cursor-pointer hover:bg-opacity-80">
              <label class="hover:bg-opacity-80 font-semibold cursor-pointer">Cancelar</label>
            </div>
          </button>
        </div>
      </div>
  ...
  methods: {
    ...
    async cancelReservation() {
      if (this.reservationIdToCancel) {
        try {
          await api.delete(`/reservations/${this.reservationIdToCancel}`);
          this.fetchReservations();
        } catch (error) {
