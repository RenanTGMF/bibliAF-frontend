<template>
    <div class="p-6 bg-gradient-to-b from-blue-50 to-blue-200 min-h-screen">
      ...
      <h1 class="text-4xl font-bold text-center mb-8 text-gray-800">{{ genreName }}</h1>
...
import api from '../api';
  ...
  export default {
    ...
    methods: {
      async fetchBooks() {
        try {
          const response = await api.get(`/books/find/genre/${this.$route.params.id}?page=${this.currentPage}&size=${this.pageSize}`);
          this.books = response.data?._embedded?.bookDtoList || [];
          this.totalPages = response.data?.page?.totalPages || 0;
          this.genreName = response.data?._embedded?.bookDtoList[0]?.genre?.name || '';
        } catch (error) {
          console.error(error);
        }
      },
